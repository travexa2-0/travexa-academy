...
**Última actualización: 20 Julio 2026 — Sesión 40.** El detalle de cada sesión vive únicamente en la tabla de "Estado actual del proyecto" (✅ Completado) y, si construyó una feature, en su sección dedicada — no se repite acá.
...
**Schema (migración PROPUESTA — NO aplicada).** `supabase/migrations/20260720000000_vivencial_destino.sql`, aditiva sobre `academy_courses`:
...
- Excepción explícita a la regla #12 (Claude aplica las migraciones directo): **esta la pidió Nico como PROPUESTA**, queda commiteada pero **sin correr**. Los vivenciales existentes que solo tienen País no cambian: `vivencial_destino` NULL + flag en `false` (default) → el hero sigue mostrando País tal cual.
...
**Pendiente.** (1) **Aplicar la migración** `20260720000000_vivencial_destino.sql` cuando Nico dé el OK — hasta entonces las columnas no existen en prod y cargar un Destino/tildar el toggle fallaría al guardar. (2) Prueba visual en prod (principio #13): cargar un Destino, tildar el toggle, ver el hero con Destino grande + País como tag, y los dos filtros del listado funcionando por separado y combinados.
