    ...
    <AnimatePresence>
      {open && (
        ...
        >
          ...
          >
            <button onClick={onClose} disabled={reserving} className="absolute top-4 right-4 w-[30px] h-[30px] rounded-full flex items-center justify-center border" style={{ background: 'var(--card)', borderColor: 'var(--line)', color: 'var(--text-3)' }} aria-label="Cerrar">
            ...
            <p className="text-xs text-center mt-3" style={{ color: 'var(--text-3)' }}>
              La reserva no cobra nada ahora. El pago es por transferencia, coordinado con Yesica.
            </p>
          </motion.div>
        </motion.div>
      )}
