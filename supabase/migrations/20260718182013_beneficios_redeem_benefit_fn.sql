-- Beneficios — función de canje atómico redeem_benefit()
-- Ya aplicada en producción. Versionado para paridad del repo.
-- La versión final (idéntica a la que corre en prod) se re-declara en
-- 20260718182139_beneficios_fix_tipos_transaccion.sql. Acá va la versión inicial.
-- SECURITY DEFINER + EXECUTE solo para service_role (el front nunca la llama directo).

CREATE OR REPLACE FUNCTION public.redeem_benefit(p_user_id uuid, p_benefit_id uuid, p_cantidad_chances integer DEFAULT 1)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  INSERT INTO academy_credit_redemptions
    (user_id, tipo, creditos_consumidos, benefit_id, course_id, cantidad_chances,
     descuento_tipo, descuento_valor, estado, usado_at, descripcion)
  VALUES
    (p_user_id, v_benefit.tipo, v_costo, v_benefit.id, v_benefit.course_id,
     ...
     v_desc_tipo, v_benefit.descuento_valor, v_estado, v_usado_at, v_benefit.titulo)
...
REVOKE ALL ON FUNCTION public.redeem_benefit(uuid, uuid, integer) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.redeem_benefit(uuid, uuid, integer) TO service_role;
